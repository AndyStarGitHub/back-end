...
def register_exception_handlers(app: FastAPI) -> None:
    ...
    async def _401(_req, exc: NotAuthenticated):
        return JSONResponse(status_code=401, content={"detail": str(exc) or "Not authenticated"})

    @app.exception_handler(InvalidCredentials)
    async def _401_creds(_req, exc: InvalidCredentials):
        return JSONResponse(status_code=401, content={"detail": str(exc) or "Invalid credentials"})

    @app.exception_handler(TokenInvalid)
    async def _401_token(_req, exc: TokenInvalid):
        return JSONResponse(status_code=401, content={"detail": str(exc) or "Invalid token"})
